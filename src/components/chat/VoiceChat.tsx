...
export function VoiceChatModal({ channelId, creatorName, creatorAvatar, isOpen, onClose }: VoiceChatModalProps) {
        ...
        <ScrollArea className="flex-1 min-h-[250px] max-h-[400px]" ref={scrollRef}>
          <div className="p-4 space-y-3">
            {conversationHistory.map((item, index) => (
              <TranscriptBubble
                key={`${item.timestamp}-${item.role}-${index}`}
        ...
        return (
          <div
            ...
            style={{ 
              ...
              height: isActive && (isSpeaking || isListening) 
                ? undefined 
                : '4px',
            }}
          />
