...
export function useVoiceSession({
  ...
  const startSession = useCallback(async () => {
    ...
    try {
      ...
      console.log(
        "[Voice] Got ephemeral key, expires at:",
      ...
      if (!sdpResponse.ok) {
        ...
        console.error(
          "[Voice] OpenAI SDP exchange failed:",
          sdpResponse.status,
          errorText,
        );
    ...
    (event: any) => {
      ...
      switch (type) {
        ...
        case "conversation.item.input_audio_transcription.completed":
          ...
          if (userText.trim()) {
            setState((prev) => {
              ...
              const isDuplicate = conversationHistoryRef.current.some(
                msg => msg.role === "user" && msg.content === userText
              );
        ...
        case "response.audio_transcript.delta":
          ...
          setState((prev) => ({
            ...prev,
            aiTranscript: prev.aiTranscript + deltaText,
            status: "connected",
          }));
          break;

        case "response.audio_transcript.done":
          console.log("[Voice] AI finished speaking", event);
          const responseId = event.response_id || event.item_id;
          if (responseId && lastProcessedResponseRef.current === responseId) {
            console.log("[Voice] Skipping duplicate response");
            break;
          }
          if (responseId) {
          ...
          setState((prev) => {
            if (prev.aiTranscript.trim()) {
              // Check if this message already exists in history
              const isDuplicate = conversationHistoryRef.current.some(
                msg => msg.role === "assistant" && msg.content === prev.aiTranscript
              );
              ...
              if (!isDuplicate) {
                ...
                conversationHistoryRef.current.push(newAiMessage);
                setConversationHistory([...conversationHistoryRef.current]);
              }
            }
            return { ...prev, aiTranscript: "", status: "listening" };
          });
          break;
